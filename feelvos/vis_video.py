...
flags.DEFINE_string('vis_logdir', "./datasets/davis17/exp/eval_on_val_set/eval", 'Where to write the event logs.')

flags.DEFINE_string('checkpoint_dir', "./models/feelvos_davis17_trained", 'Directory of model checkpoints.')

flags.DEFINE_integer('output_stride', 8, 'The ratio of input to output spatial resolution.')

flags.DEFINE_string('dataset', 'davis_2017', 'Name of the segmentation dataset.')

flags.DEFINE_string('vis_split', 'val', 'Which split of the dataset used for visualizing results')

flags.DEFINE_string('dataset_dir', './datasets/davis17/tfrecord',
                    'Where the dataset resides.')

flags.DEFINE_integer('num_vis_examples', -1,
                     'Number of examples for visualization. If -1, use all ' 'samples in the vis data.')

flags.DEFINE_multi_integer('atrous_rates', [12, 24, 36], 'Atrous rates for atrous spatial pyramid pooling.')

flags.DEFINE_bool('save_segmentations', True,
                  'Whether to save the segmentation masks as png images. Might be slow on cns.')

flags.DEFINE_bool('save_embeddings', False, 'Whether to save the embeddings as pickle. Might be slow on cns.')

flags.DEFINE_bool('eval_once_and_quit', False, 'Whether to just run the eval a single time and quit afterwards. '
                                               'Otherwise, the eval is run in a loop with new checkpoints.')

flags.DEFINE_boolean('first_frame_finetuning', False, 'Whether to only sample the first frame for fine-tuning.')
...
def create_predictions_fast(samples, reference_labels, first_frame_img,
    ...
    def predict(args, img):
        ...
        predictions, softmax_probabilities, embeddings = model.predict_labels(
            ...
            use_softmax_feedback=FLAGS.use_softmax_feedback,
            initial_softmax_feedback=last_softmax_probabilities,
            embedding_seg_feature_dimension=FLAGS.embedding_seg_feature_dimension,
            embedding_seg_n_layers=FLAGS.embedding_seg_n_layers,
            embedding_seg_kernel_size=FLAGS.embedding_seg_kernel_size,
            embedding_seg_atrous_rates=FLAGS.embedding_seg_atrous_rates,
            also_return_softmax_probabilities=True,
            num_frames_per_video=1,
            normalize_nearest_neighbor_distances=FLAGS.normalize_nearest_neighbor_distances,
            also_attend_to_previous_frame=FLAGS.also_attend_to_previous_frame,
            use_local_previous_frame_attention=FLAGS.use_local_previous_frame_attention,
            previous_frame_attention_window_size=FLAGS.previous_frame_attention_window_size,
            use_first_frame_matching=FLAGS.use_first_frame_matching,
...
def main(unused_argv):
    ...
    with g.as_default():
        ...
        while True:
            ...
            all_ious = []
            with sv.managed_session(FLAGS.master, config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True)),
